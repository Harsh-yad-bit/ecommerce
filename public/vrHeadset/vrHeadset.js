...
window.addEventListener('load', e => {
    ...
    createApp({
        ...
        sceneURL: params.load || 'vrHeadset.gltf',
...
function runCode(app, puzzles) {
    // Listen for color change clicks coming from your React Product.jsx component
    window.addEventListener('message', function(event) {
        if (event.data && event.data.type === 'CHANGE_MATERIAL') {
            const objNames = event.data.objects;
            const matName = event.data.material;

            let matToAssign = null;
            app.scene.traverse(function(node) {
                if (node.material && node.material.name === matName) {
                    matToAssign = node.material;
                }
            });
